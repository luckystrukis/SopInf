...
for x in e3:
    if x != e3[0]:
        if x.find('small') != None:
            ...
            x2 = x.find_all('td')
            if i == 0:
                diccionario = {'pais': str(x1[1].get_text()), 'oro': str(x2[2].get_text()), 'plata': str(x2[3].get_text()), 'bronce': str(x2[4].get_text()), 'total': str(x2[5].get_text())}
                dataframe = pd.DataFrame(diccionario, columns=[
                                            'index', 'pais', 'oro', 'plata', 'bronce', 'total'], index=[i])
            else:
                diccionario = {'pais': str(x1[1].get_text()), 'oro': str(x2[2].get_text()), 'plata': str(x2[3].get_text()), 'bronce': str(x2[4].get_text()), 'total': str(x2[5].get_text())}
